...
static BOOL ConvertPath_( PList lstPath, const char * szAbsolutePath )
{
	...
	dwLen = 1;
	szDirItem = (char *)malloc( dwLen + 1 );
	if ( 0 == szDirItem )
	{
		ClearListWithPointerValue_( lstPath );
		return FALSE;
	}
...
int  LmnCreateFolder( IN const char * szPathName, BOOL bIgnoreExisted /*= TRUE*/ )
{
	...
	int nCount = MultiByteToWideChar( CODE_PAGE, 0, szPath, dwLen, wszPath, LMN_MAX_FILE_PATH - 1 );
	if ( 0 == nCount )
	{
		return LMNX_WRONG_PARAMS;
	}
	...
	{
		return LMNX_FILE_EXISTS;
	}
	else if ( E_FILE_TYPE_FOLDER == eFileType )
	{
		if ( !bIgnoreExisted )
		{
			return LMNX_FOLDER_EXISTS;
		}
		else
		{
			return LMNX_OK;
...
FILE * LmnOpenFile ( const char * szFile, const char * szMode )
{
	...
	MultiByteToWideChar( CODE_PAGE,  0, szFile, strlen(szFile),  wszFile, LMN_MAX_FILE_PATH - 1 );
	MultiByteToWideChar( CODE_PAGE,  0, szMode, strlen(szMode),  wszMode, (sizeof(wszMode)/sizeof(WCHAR)) - 1 );
...
int   LmnDeleteFile( IN const char * szPathName )
{
	...
	if ( E_FILE_TYPE_NONE == eFileType )
	{
		return LMNX_FILE_DOES_NOT_EXIST;
	}
...
int   LmnCopyFile( IN const char * szOldPathName, IN const char * szNewPathName, IN BOOL bOverWrite /*= TRUE*/ )
{
	...
	dwOldLen = strlen(szOldPath);
	dwNewLen = strlen(szNewPath);

#ifdef WIN32
	wchar_t  wszOldPath[LMN_MAX_FILE_PATH] = { L'\0' };
	int nCount = 0;
	nCount = MultiByteToWideChar( CODE_PAGE, 0, szOldPath, dwOldLen, wszOldPath, LMN_MAX_FILE_PATH - 1 );
	if ( 0 == nCount )
	{
		DeinitList( lstOld );
		DeinitList( lstNew );
	...
	nCount = MultiByteToWideChar( CODE_PAGE, 0, szNewPath, dwNewLen, wszNewPath, LMN_MAX_FILE_PATH - 1 );
	if ( 0 == nCount )
	{
		DeinitList( lstOld );
		DeinitList( lstNew );
...
#ifdef WIN32
		return ::CopyFileW( pOldPath, pNewPath, !bOverWrite ) ? LMNX_OK : LMNX_UNKNOW;
#else
		return CopySingleFile_( pOldPath, pNewPath, TRUE ) ? LMNX_OK : LMNX_UNKNOW;
#endif
	}
	else
	{
...
#ifdef WIN32
		return (::MoveFileW( pOldPath, pNewPath ) != 0 ? LMNX_OK : LMNX_UNKNOW );
#else
		return MoveSingleFile_( pOldPath, pNewPath, TRUE ) ? LMNX_OK : LMNX_UNKNOW;
#endif
	}
	else
	{
...
static  int  TraverseDirectoryOrderly( const F_CHAR * folder, TraverseDirectoryCallback pfnCb, void * pParam0, void * pParam1 )
{
	...
	if ( INVALID_HANDLE_VALUE != handle )
	{
		while( FindNextFileW( handle, &data ) )
		{
			...
			wchar_t * pFileName = (wchar_t *)malloc( sizeof(wchar_t) * 260 );
			if ( 0 == pFileName )
			{
				ClearListWithPointerValue_(lstDirs);
				ClearListWithPointerValue_(lstFiles);
				DeinitList( lstDirs );
				DeinitList( lstFiles );
				return -1;
			}
	...
	while ( dirent_ )
	{
		...
		if ( !(0 == strcmp( dirent_->d_name, "." ) || 0 == strcmp( dirent_->d_name, ".." )) )
		{
			...
			char * pFileName = (char *)malloc( sizeof(char) * 260 );
			if ( 0 == pFileName )
			{
				ClearListWithPointerValue_(lstDirs);
				ClearListWithPointerValue_(lstFiles);
...
static BOOL  TraverseGetAllFiles_( const F_CHAR * full_path, BOOL bFolder, const F_CHAR * file_name, void * pParam0, void * pParam1 )
{
	...
	TSubFileNode * pFileNode = (TSubFileNode *)malloc( sizeof(TSubFileNode) );
	if ( 0 == pFileNode )
	{
		return FALSE;
	}
	...
	pFileNode->pFileName = (F_CHAR *)malloc( sizeof(F_CHAR) * (dwFileNameLen + 1) );
...
static BOOL WriteFile_( PTreeNode pNode, const void * buf, DWORD * pdwLeftSize, TStreamHandle * pStreamHandle, DWORD * pdwWriteFileSize )
{
	...
	if ( SUB_FILE_TRANSFER_STATE_RECEIVED_FILE_NAME_STRING == pStreamHandle->eBufState )
	{
		...
		while( p1 )
		{
			...
			BOOL bFind = FALSE;
			PTreeNode pChildNode = GetFirstChildTreeNode( pNode );
			while( pChildNode )
			{
		...
		TSubFileNode * pNewFile = (TSubFileNode *)malloc( sizeof(TSubFileNode) );
		if ( 0 == pNewFile )
		{
			return FALSE;
		}
		...
		pNewFile->pFileName = (F_CHAR *)malloc( sizeof(F_CHAR) * (dwFileNameLen + 1) );
										   ...
										   BOOL bOverWrite /*= FALSE*/, TFileStreamPos * ptPos /*= 0*/ )
{
	...
	if ( bRead )
	{
		...
		// 分配句柄内存
		TStreamHandle * pStreamHandle = (TStreamHandle *)malloc( sizeof(TStreamHandle) );
		if ( 0 == pStreamHandle )
		{
			return 0;
		}
		...
		pStreamHandle->pFilePath = (F_CHAR *)malloc( sizeof(F_CHAR) * (dwPathLen + 1) );
		...
		else
		{
			...
			pStreamHandle->pCurNode = GetFirstChildTreeNode( pStreamHandle->pFiles );
		}
...
//            数据：   长度(4字节，网络序)+纯数据
int ReadFileStream( HFileStream h, void * buf, DWORD * pdwBufSize, OUT DWORD * pdwReadFileSize /*= 0*/ )
{
