...
extern char *  GetStringFromUrl(char * szDest, DWORD dwDestSize, const char * szUrl);
extern void    InitGLock();
...
public:
                   ...
                   EFormatDataType eType =  FORMAT_DATA_TYPE_NORMAL, BOOL bUtf8 = FALSE )
    {
        ...
        if ( FORMAT_DATA_TYPE_UPLOAD == m_eType )
        {
            ...
            {
                char  tmpBuf[8192];
                AnsiToUtf8( tmpBuf, 8192, m_sFileName.c_str());
                m_sFileName = tmpBuf;
            }
        }
        else
        {
            ...
            {
                char  tmpBuf[8192];
                AnsiToUtf8( tmpBuf, 8192, sValue.c_str() );
                m_sValue = tmpBuf;
            }
            else
            {
